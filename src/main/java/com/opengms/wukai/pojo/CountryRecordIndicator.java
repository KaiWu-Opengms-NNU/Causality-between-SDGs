...
public class CountryRecordIndicator {
    ...
    public void LoadData(List<List<String>> SDG1,List<List<String>> SDG2,List<List<String>> SDG3,List<List<String>> SDG4,
        ...
        for(int i=0;i<this.SeriesCode.size();i++){
            String DirectionNow=this.Direction.get(i);
            switch (this.getGoals().get(i)){
                case "1":
                    SearchData(i,DirectionNow,SDG1,StartYear,EndYear);
                    break;
                case "2":
                    SearchData(i,DirectionNow,SDG2,StartYear,EndYear);
                    break;
                case "3":
                    SearchData(i,DirectionNow,SDG3,StartYear,EndYear);
                    break;
                case "4":
                    SearchData(i,DirectionNow,SDG4,StartYear,EndYear);
                    break;
                case "5":
                    SearchData(i,DirectionNow,SDG5,StartYear,EndYear);
                    break;
                case "6":
                    SearchData(i,DirectionNow,SDG6,StartYear,EndYear);
                    break;
                case "7":
                    SearchData(i,DirectionNow,SDG7,StartYear,EndYear);
                    break;
                case "8":
                    SearchData(i,DirectionNow,SDG8,StartYear,EndYear);
                    break;
                case "9":
                    SearchData(i,DirectionNow,SDG9,StartYear,EndYear);
                    break;
                case "10":
                    SearchData(i,DirectionNow,SDG10,StartYear,EndYear);
                    break;
                case "11":
                    SearchData(i,DirectionNow,SDG11,StartYear,EndYear);
                    break;
                case "12":
                    SearchData(i,DirectionNow,SDG12,StartYear,EndYear);
                    break;
                case "13":
                    SearchData(i,DirectionNow,SDG13,StartYear,EndYear);
                    break;
                case "14":
                    SearchData(i,DirectionNow,SDG14,StartYear,EndYear);
                    break;
                case "15":
                    SearchData(i,DirectionNow,SDG15,StartYear,EndYear);
                    break;
                case "16":
                    SearchData(i,DirectionNow,SDG16,StartYear,EndYear);
                    break;
                case "17":
                    SearchData(i,DirectionNow,SDG17,StartYear,EndYear);
                    break;
            }
        }
    }
...
//    }
    ...
    private void SearchData(int k, String DirectionN, List<List<String>> SDGX, int StartYear, int EndYear){
        ...
        if(oneYear.size()>0){
            ...
            ProcessingData=DataUniform2(ProcessingData,DirectionN);
    ...
    //数据归一化（Min-Max归一化）
    private List<YearRecord> DataUniform2(List<YearRecord> originalData, String Direction){
        for (int i=originalData.size()-1;i>=0;i--){
            try{
                Double.parseDouble(originalData.get(i).getValue());
            }catch (Exception e){
        ...
        try{
            ...
            if(Direction.equals("Forward"))
            {
                ...
                if(Max>Min){
                    for(int i=0;i<originalData.size();i++){
                        YearRecord oneYear=new YearRecord();
                        oneYear.setValue(String.valueOf(100*(Max-Double.parseDouble(originalData.get(i).getValue()))/(Max-Min)));
                        oneYear.setYear(originalData.get(i).getYear());
                        result.add(oneYear);
                    }
                }
                ...
                //说明是中性指标
                //判断出中性的阈值
                String [] MiddleValueList=Direction.split("_");
                Double MiddleValue=Double.parseDouble(MiddleValueList[1]);
                if(Max>MiddleValue&&MiddleValue>Min){
                    for(int i=0;i<originalData.size();i++){
                        YearRecord oneYear=new YearRecord();
                        oneYear.setValue(String.valueOf(  100*(1-Math.abs((Double.parseDouble(originalData.get(i).getValue())-MiddleValue)/(Max-Min)))  ));
                        oneYear.setYear(originalData.get(i).getYear());
                        result.add(oneYear);
                    }
                }
