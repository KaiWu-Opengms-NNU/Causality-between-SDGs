import com.opengms.wukai.pojo.CountryReclass;
import com.opengms.wukai.pojo.HaveDataSDG;
import com.opengms.wukai.pojo.Result.*;

import com.opengms.wukai.pojo.Result.Indicator.*;
import com.opengms.wukai.pojo.Result.NetWork.CentralGraph;
import com.opengms.wukai.pojo.Result.NetWork.CountryNandR;
...
public class StaticsAnalysis {
    public static String Path="src/test/result_spatial/";
    public static String OutPath="src/test/result_spatial_per/";
    public static String OutPath2="src/test/result_spatial_perlocation/";
    public static String OutPath3="src/test/result_spatial_prioritize/";
    public static String OutPath4="src/test/result_spatial_SP/";
    ...
    public static void main(String[] args) throws IOException {
        ...
        //载入标准分区文档
        List<List<String>> CorrespondenceRegionAndCountry = new OperateExcel().readExcel(new File("InputData/ReclassRegionLoading.xlsx"));
        //载入分区及国家尺度
        ...
        //转化成类列表
        for (int i = 1; i < AnalysisResult.size(); i++) {
            ...
            for (int j = 1; j < 18; j++) {
                ...
                for (int countryI = 0; countryI < CountryAndRegionList_Goals.size(); countryI++) {
                    ...
                    if (Scale == 3) {
                        ...
                        if((WorldInteraction!=null&&RegionInteraction!=null)&&CountryInteraction!=null)
                        {
                            oneRegion.setChangeFromGlobalToNational_Strength(TempCalculate[0]-TempCalculate[2]);
                            oneRegion.setChangeFromRegionalToNational_Strength(TempCalculate[0]-TempCalculate[1]);
                        }
                        if((WorldInteraction!=null&&RegionInteraction==null)&&CountryInteraction!=null)
                        {
                            oneRegion.setChangeFromGlobalToNational_Strength(TempCalculate[0]-TempCalculate[1]);
                        }
                        if((WorldInteraction==null&&RegionInteraction!=null)&&CountryInteraction!=null)
                        {
                            oneRegion.setChangeFromRegionalToNational_Strength(TempCalculate[0]-TempCalculate[1]);
        ...
        //三个尺度需要有任一一个尺度大于百分之0%才可以用于进一步的分析（更新之后不用大于30%。大于0%即可）
        ...
        List<List<String>> RegionAndCountry = operateExcel.readExcel(new File("InputData/Country/CountryListFinally.xlsx"));
        List<CountryReclass> WorldCountryList = new ArrayList<>();
        for (int i = 1; i < RegionAndCountry.size(); i++) {
            CountryReclass countryCode = new CountryReclass();
            countryCode.setGeoAreaName(RegionAndCountry.get(i).get(0));
            countryCode.setGeoAreaShort(RegionAndCountry.get(i).get(1));
            countryCode.setGeoAreaCodeID(RegionAndCountry.get(i).get(2));
            countryCode.setGeoIncome(RegionAndCountry.get(i).get(4));
            countryCode.setGeoTag(RegionAndCountry.get(i).get(5));
            countryCode.setGeoParentRegionIII(RegionAndCountry.get(i).get(7));
            WorldCountryList.add(countryCode);
        }
        List<List<String>> ReClassCountry= operateExcel.readExcel(new File("InputData/CountryReclass/CountryProfileReclass.xlsx"));
        //通过List组合读取数据
        Map<String,Double> AttributeCountry=new HashMap<>();
        SDGIndicatorTool Tools=new SDGIndicatorTool();
        for(int PathI=1;PathI<ReClassCountry.size();PathI++)
        {
            String ReadPath="InputData/CountryReclass/"+ReClassCountry.get(PathI).get(0)+"/"+ReClassCountry.get(PathI).get(1)+".xlsx";
            List<List<String>> CountryFeatureContent= operateExcel.readExcel(new File(ReadPath));
            for(int CFI=1;CFI<CountryFeatureContent.size();CFI++)
            {

                double TempContentValue=-1;
                for(int j=4;j<CountryFeatureContent.get(CFI).size();j++){
                    try{
                        TempContentValue=Double.parseDouble(CountryFeatureContent.get(CFI).get(j));
                    }
                    catch(NumberFormatException ignored)
                    {}
                }
                AttributeCountry.put(CountryFeatureContent.get(CFI).get(1),TempContentValue);
            }

            List<Double> SortedValues=new ArrayList<>(AttributeCountry.values());
            Collections.sort(SortedValues);
            double min=SortedValues.get(0);
            double max=SortedValues.get(SortedValues.size()-1);
            double q1=Tools.calculateQuantile(SortedValues,0.25);
            double q2=Tools.calculateQuantile(SortedValues,0.50);
            double q3=Tools.calculateQuantile(SortedValues,0.75);
            for(int i=0;i<WorldCountryList.size();i++){
                String CountryName=WorldCountryList.get(i).getGeoAreaShort();
                double CountryValue=AttributeCountry.get(CountryName);
                if(CountryValue>=min&&CountryValue<=q1)
                {
                    WorldCountryList.get(i).getAttributeCountry().put(ReClassCountry.get(PathI).get(1),"L");
                }
                if(CountryValue>q1&&CountryValue<=q2)
                {
                    WorldCountryList.get(i).getAttributeCountry().put(ReClassCountry.get(PathI).get(1),"LM");
                }
                if(CountryValue>q2&&CountryValue<=q3)
                {
                    WorldCountryList.get(i).getAttributeCountry().put(ReClassCountry.get(PathI).get(1),"UM");
                }
                if(CountryValue>q3&&CountryValue<=max)
                {
                    WorldCountryList.get(i).getAttributeCountry().put(ReClassCountry.get(PathI).get(1),"H");
                }
        ...
        {
            int scale=AllSDGList.get(i).getScale();
            if(scale==3)
            {
                //判断尺度变化的方向
                String GlobalSDGInteraction= AllSDGList.get(i).getWorldInteraction();
                String NationalSDGInteraction=AllSDGList.get(i).getCountryInteraction();
                if(GlobalSDGInteraction==null||NationalSDGInteraction==null)
                {}else{
                    if(GlobalSDGInteraction.equals("NE")||NationalSDGInteraction.equals("NE"))
                    {
                        AllSDGList.get(i).setRelatedNeutralizationGN(0);
                        AllSDGList.get(i).setChangeFromGlobalToNational(Tools.JudgeScaleChangeByRelatedNeutralization(GlobalSDGInteraction,NationalSDGInteraction));
                    }else
                    {
                        AllSDGList.get(i).setRelatedNeutralizationGN(1);
                        AllSDGList.get(i).setChangeFromGlobalToNational(Tools.JudgeScaleChangeByNoRelatedNeutralization(GlobalSDGInteraction,NationalSDGInteraction));
                    }
                }
                String RegionalSDGInteraction=AllSDGList.get(i).getRegionInteraction();
                if(RegionalSDGInteraction==null||NationalSDGInteraction==null)
                {}else{
                    if(RegionalSDGInteraction.equals("NE")||NationalSDGInteraction.equals("NE"))
                    {
                        AllSDGList.get(i).setRelatedNeutralizationRN(0);
                        AllSDGList.get(i).setChangeFromRegionalToNational(Tools.JudgeScaleChangeByRelatedNeutralization(RegionalSDGInteraction,NationalSDGInteraction));
                    }
                    else
                    {
                        AllSDGList.get(i).setRelatedNeutralizationRN(1);
                        AllSDGList.get(i).setChangeFromRegionalToNational(Tools.JudgeScaleChangeByNoRelatedNeutralization(RegionalSDGInteraction,NationalSDGInteraction));
                    }
                }
                //判断国家的属性
                String AreaID=AllSDGList.get(i).getAreaID();
                CountryReclass MappingCountry=new CountryReclass();
                for (int Ai=0;Ai<WorldCountryList.size();Ai++)
                {
                    String TempID=WorldCountryList.get(Ai).getGeoAreaCodeID();
                    if(AreaID.equals(TempID)){
                        MappingCountry=WorldCountryList.get(Ai);
                    }
                }
                AllSDGList.get(i).setEconomic(MappingCountry.getGeoIncome());
                AllSDGList.get(i).setPopulation(MappingCountry.getAttributeCountry().get("Population"));
                AllSDGList.get(i).setGeoRegionParent(MappingCountry.getGeoParentRegionIII());
                AllSDGList.get(i).setGeoTag(MappingCountry.getGeoTag());
            }
        ...
        List<CountryNandR> CountryNandRAll=new ArrayList<>();
        for(int i=0;i<WorldDifferenceLevel.size();i++){
            if(WorldDifferenceLevel.get(i).getAggregation().getCountryName().equals("Liechtenstein")!=true){
                ...
                relationshipList=new SDGIndicatorTool().NoRepeatToAve(relationshipList);
                ...
                if(nodeList.size()>0){
                    ...
                    //判断Node列表的最大值和最小值
                    if(nodeList.size()>=6){
                        nodeList.get(new SDGIndicatorTool().findPositionOfThirdMaxWeightOutDegree(nodeList,1)).setMaxOrMin("null");
                        nodeList.get(new SDGIndicatorTool().findPositionOfThirdMaxWeightOutDegree(nodeList,2)).setMaxOrMin("null");
                        nodeList.get(new SDGIndicatorTool().findPositionOfThirdMaxWeightOutDegree(nodeList,3)).setMaxOrMin("null");
                        nodeList.get(new SDGIndicatorTool().findPositionOfThirdMaxWeightOutDegree(nodeList,nodeList.size()-2)).setMaxOrMin("null");
                        nodeList.get(new SDGIndicatorTool().findPositionOfThirdMaxWeightOutDegree(nodeList,nodeList.size()-1)).setMaxOrMin("null");
                        nodeList.get(new SDGIndicatorTool().findPositionOfThirdMaxWeightOutDegree(nodeList,nodeList.size())).setMaxOrMin("null");
                    }
                    //operateExcel.writeExcel(OutPath3,WorldDifferenceLevel.get(i).getAggregation().getCountryName()+"Node",NetNode.class, nodeList);

                    CountryNandR oneCountry=new CountryNandR();
                    oneCountry.setCountryID(WorldDifferenceLevel.get(i).getID());
                    oneCountry.setCountryName(WorldDifferenceLevel.get(i).getAggregation().getCountryName());
                    oneCountry.setNodeList(nodeList);
                    oneCountry.setRelationshipList(relationshipList);
                    CountryNandRAll.add(oneCountry);
                }
            }
        }
        ...
        //增补实验2——Result3
        //先依据Relationship构建网络
        List<CentralGraph> EigenvectorList=new ArrayList<>();
        for(int Wi=0;Wi<WorldDifferenceLevel.size();Wi++)
        {
            CentralGraph EOneCountry=new CentralGraph();
            String GeoArea=WorldDifferenceLevel.get(Wi).getAggregation().getCountryName();
            EOneCountry.setAreaName(GeoArea);
            EOneCountry.setAreaID(WorldDifferenceLevel.get(Wi).getAggregation().getCountryCode());
            EOneCountry.setScale(WorldDifferenceLevel.get(Wi).getLevel());

            Graph<String, DefaultWeightedEdge> GraphOfCountryFacilitation=new DefaultDirectedWeightedGraph<>(DefaultWeightedEdge.class);
            for(int Ri=0;Ri<RelationshipAll.size();Ri++)
            {
                if(GeoArea.equals(RelationshipAll.get(Ri).getAreaName())&&RelationshipAll.get(Ri).getType().equals("Facilitation"))
                {
                    String GoalX=RelationshipAll.get(Ri).getGoalX();
                    String GoalY=RelationshipAll.get(Ri).getGoalY();
                    if (!GraphOfCountryFacilitation.containsVertex(GoalX)) {
                        GraphOfCountryFacilitation.addVertex(GoalX);
                    }
                    if (!GraphOfCountryFacilitation.containsVertex(GoalY)) {
                        GraphOfCountryFacilitation.addVertex(GoalY);
                    }
                    DefaultWeightedEdge edge=GraphOfCountryFacilitation.addEdge(GoalX,GoalY);
                    GraphOfCountryFacilitation.setEdgeWeight(edge,RelationshipAll.get(Ri).getStrength());
                }
            }
            EigenvectorCentrality eigenvectorCentrality=new EigenvectorCentrality(GraphOfCountryFacilitation);
            Map<String,Double> CentralityMapFacilitation=eigenvectorCentrality.getScores();
            EOneCountry.setFacilitationGraph(GraphOfCountryFacilitation);
            boolean hasEmptyValueF=CentralityMapFacilitation.values().stream().anyMatch(v->v==null||Double.isNaN(v));
            if(hasEmptyValueF)
            {
                KatzCentrality katzCentrality=new KatzCentrality(GraphOfCountryFacilitation);
                Map<String,Double> CentralityMapFacilitation2=katzCentrality.getScores();
                System.out.println("MapValue:"+CentralityMapFacilitation2.values());
                EOneCountry.setMapFacilitationCentrality(CentralityMapFacilitation2);
            }else{
                System.out.println("MapValue:"+CentralityMapFacilitation.values());
                EOneCountry.setMapFacilitationCentrality(CentralityMapFacilitation);
            }


            Graph<String, DefaultWeightedEdge> GraphOfCountryInhibition=new DefaultDirectedWeightedGraph<>(DefaultWeightedEdge.class);
            for(int Ri=0;Ri<RelationshipAll.size();Ri++)
            {
                if(GeoArea.equals(RelationshipAll.get(Ri).getAreaName())&&RelationshipAll.get(Ri).getType().equals("Inhibition"))
                {
                    String GoalX=RelationshipAll.get(Ri).getGoalX();
                    String GoalY=RelationshipAll.get(Ri).getGoalY();
                    if (!GraphOfCountryInhibition.containsVertex(GoalX)) {

                        GraphOfCountryInhibition.addVertex(GoalX);
                    }
                    if (!GraphOfCountryInhibition.containsVertex(GoalY)) {
                        GraphOfCountryInhibition.addVertex(GoalY);
                    }
                    DefaultWeightedEdge edge=GraphOfCountryInhibition.addEdge(GoalX,GoalY);
                    GraphOfCountryInhibition.setEdgeWeight(edge,RelationshipAll.get(Ri).getStrength());
                }
            }
            EigenvectorCentrality eigenvectorCentralityInhibition=new EigenvectorCentrality(GraphOfCountryInhibition);
            Map<String,Double> CentralityMapInhibition=eigenvectorCentralityInhibition.getScores();
            EOneCountry.setInhibitionGraph(GraphOfCountryInhibition);
            boolean hasEmptyValueI=CentralityMapInhibition.values().stream().anyMatch(v->v==null||Double.isNaN(v));
            if(hasEmptyValueI)
            {
                KatzCentrality katzCentrality=new KatzCentrality(GraphOfCountryInhibition);
                Map<String,Double> CentralityMapInhibition2=katzCentrality.getScores();
                System.out.println("MapValue:"+CentralityMapInhibition2.values());
                EOneCountry.setMapInhibitionCentrality(CentralityMapInhibition2);
            }else{
                System.out.println("MapValue:"+CentralityMapInhibition.values());
                EOneCountry.setMapInhibitionCentrality(CentralityMapInhibition);
            }

            EigenvectorList.add(EOneCountry);
        }
        List<CentralGraph> EigenvectorList2=new ArrayList<>();
        for(int Ei=0;Ei<EigenvectorList.size();Ei++)
        {
            if(EigenvectorList.get(Ei).getScale()==2){
                EigenvectorList2.add(EigenvectorList.get(Ei));
            }
        }
        System.out.println("Success");
        ...
        //首先依据前三强度判断出哪些是优先促进作用目标 哪些是优先抑制作用目标
        List<ArcmapForNetwork> ArcmapForNetworkList=new ArrayList<>();
        for(int Wi=0;Wi<WorldDifferenceLevel.size();Wi++)
        {
            if(EigenvectorList.get(Wi).getMapFacilitationCentrality().isEmpty())
            {
                System.out.println("网络是空的");
            }else
            {
                for(Map.Entry<String,Double> entry: EigenvectorList.get(Wi).getMapFacilitationCentrality().entrySet())
                {
                    ArcmapForNetwork oneCountry_oneSDG=new ArcmapForNetwork();
                    oneCountry_oneSDG.setAreaName(WorldDifferenceLevel.get(Wi).getAggregation().getCountryName());
                    oneCountry_oneSDG.setAreaAb(WorldDifferenceLevel.get(Wi).getAggregation().getCountryAb());
                    oneCountry_oneSDG.setAreaID(WorldDifferenceLevel.get(Wi).getID());
                    oneCountry_oneSDG.setGoal(entry.getKey());
                    oneCountry_oneSDG.setScale(WorldDifferenceLevel.get(Wi).getLevel());
                    oneCountry_oneSDG.setGraphWeight(entry.getValue());
                    oneCountry_oneSDG.setType("Facilitation");
                    oneCountry_oneSDG.setGeoRegionID(WorldDifferenceLevel.get(Wi).getParentID());
                    CountryReclass MappingCountry=new CountryReclass();
                    for (int Ai=0;Ai<WorldCountryList.size();Ai++)
                    {
                        String TempID=WorldCountryList.get(Ai).getGeoAreaCodeID();
                        if(String.valueOf(oneCountry_oneSDG.getAreaID()).equals(TempID)){
                            MappingCountry=WorldCountryList.get(Ai);
                        }
                    }
                    oneCountry_oneSDG.setEconomic(MappingCountry.getGeoIncome());
                    oneCountry_oneSDG.setPopulation(MappingCountry.getAttributeCountry().get("Population"));
                    ArcmapForNetworkList.add(oneCountry_oneSDG);
                }
            }
            if(EigenvectorList.get(Wi).getMapInhibitionCentrality().isEmpty())
            {
                System.out.println("网络是空的");
            }else{
                for(Map.Entry<String,Double> entry: EigenvectorList.get(Wi).getMapInhibitionCentrality().entrySet())
                {
                    ArcmapForNetwork oneCountry_oneSDG=new ArcmapForNetwork();
                    oneCountry_oneSDG.setAreaName(WorldDifferenceLevel.get(Wi).getAggregation().getCountryName());
                    oneCountry_oneSDG.setAreaAb(WorldDifferenceLevel.get(Wi).getAggregation().getCountryAb());
                    ...
                    oneCountry_oneSDG.setScale(WorldDifferenceLevel.get(Wi).getLevel());
                    oneCountry_oneSDG.setGoal(entry.getKey());
                    oneCountry_oneSDG.setGraphWeight(0-entry.getValue());
                    oneCountry_oneSDG.setType("Inhibition");
                    oneCountry_oneSDG.setGeoRegionID(WorldDifferenceLevel.get(Wi).getParentID());
                    CountryReclass MappingCountry=new CountryReclass();
                    for (int Ai=0;Ai<WorldCountryList.size();Ai++)
                    {
        ...
        int ID=ArcmapForNetworkList.get(0).getAreaID();
        List<ArcmapForNetworkJudge> TempJudge=new ArrayList<>();
        for(int Ni=0;Ni<ArcmapForNetworkList.size();Ni++)
        {
            //首先是先把同一个国家的信息获取
            if(ArcmapForNetworkList.get(Ni).getAreaID()==ID)
            {
                ArcmapForNetworkJudge oneForPrioritize=new ArcmapForNetworkJudge();
                oneForPrioritize.setListIndex(Ni);
                oneForPrioritize.setOutWeight(ArcmapForNetworkList.get(Ni).getGraphWeight());
                oneForPrioritize.setTag("NP");
                TempJudge.add(oneForPrioritize);
            }
            else
            {
                ID=ArcmapForNetworkList.get(Ni).getAreaID();
                List<ArcmapForNetworkJudge> positiveList = TempJudge.stream()
                        .filter(e -> e.getOutWeight() > 0)
                        .sorted(Comparator.comparingDouble(ArcmapForNetworkJudge::getOutWeight).reversed())
                        .collect(Collectors.toList());

                int fpCount = Math.min(3, positiveList.size());
                for (int i = 0; i < fpCount; i++) {
                    positiveList.get(i).setTag("FP");
                }

                // 处理OutWeight < 0的元素，按升序取前三个设为IP
                List<ArcmapForNetworkJudge> negativeList = TempJudge.stream()
                        .filter(e -> e.getOutWeight() < 0)
                        .sorted(Comparator.comparingDouble(ArcmapForNetworkJudge::getOutWeight))
                        .collect(Collectors.toList());

                int ipCount = Math.min(3, negativeList.size());
                for (int i = 0; i < ipCount; i++) {
                    negativeList.get(i).setTag("IP");
                }
                if(positiveList.size()>0)
                {
                    for(int pi=0;pi<positiveList.size();pi++)
                    {
                        ArcmapForNetworkList.get(positiveList.get(pi).getListIndex()).setPrioritize(positiveList.get(pi).getTag());
                    }
                }
               if(negativeList.size()>0)
               {
                   for(int pi=0;pi<negativeList.size();pi++){
                        ArcmapForNetworkList.get(negativeList.get(pi).getListIndex()).setPrioritize(negativeList.get(pi).getTag());
                   }
               }
               //重置并赋值
               TempJudge=new ArrayList<>();
               ArcmapForNetworkJudge oneForPrioritize=new ArcmapForNetworkJudge();
               oneForPrioritize.setListIndex(Ni);
               oneForPrioritize.setOutWeight(ArcmapForNetworkList.get(Ni).getGraphWeight());
               oneForPrioritize.setTag("NP");
        ...
        System.out.println("优先促进和抑制目标已经识别完成");
        List<ArcmapForNetwork> ArcmapForNetworkListForRegion=new ArrayList<>();


        //在分区尺度上提取目标
        for(int Ai=0;Ai<ArcmapForNetworkList.size();Ai++)
        {
            if(ArcmapForNetworkList.get(Ai).getScale()==2){
                ArcmapForNetworkListForRegion.add(ArcmapForNetworkList.get(Ai));
            }
        }
        operateExcel.writeExcel(OutPath3,"RegionPrioritySDGs",ArcmapForNetwork.class,ArcmapForNetworkListForRegion);



        //以国家为单位提取国家尺度对比全球和区域尺度不一样的地方
        List<ArcmapForNetworkByCountry> CountryComparedGlobe=new ArrayList<>();
        List<ArcmapForNetworkByCountry> CountryComparedRegion=new ArrayList<>();
        for(int Ai=0;Ai<WorldCountryList.size();Ai++)
        {
            ArcmapForNetworkByCountry CCG=new ArcmapForNetworkByCountry();
            CCG.InitClass(Integer.parseInt(WorldCountryList.get(Ai).getGeoAreaCodeID()),ArcmapForNetworkList);
            CCG.setSDG1(CCG.JudgeSDGTypeAndChange("1",1,ArcmapForNetworkList));
            CCG.setSDG2(CCG.JudgeSDGTypeAndChange("2",1,ArcmapForNetworkList));
            CCG.setSDG3(CCG.JudgeSDGTypeAndChange("3",1,ArcmapForNetworkList));
            CCG.setSDG4(CCG.JudgeSDGTypeAndChange("4",1,ArcmapForNetworkList));
            CCG.setSDG5(CCG.JudgeSDGTypeAndChange("5",1,ArcmapForNetworkList));
            CCG.setSDG6(CCG.JudgeSDGTypeAndChange("6",1,ArcmapForNetworkList));
            CCG.setSDG7(CCG.JudgeSDGTypeAndChange("7",1,ArcmapForNetworkList));
            CCG.setSDG8(CCG.JudgeSDGTypeAndChange("8",1,ArcmapForNetworkList));
            CCG.setSDG9(CCG.JudgeSDGTypeAndChange("9",1,ArcmapForNetworkList));
            CCG.setSDG10(CCG.JudgeSDGTypeAndChange("10",1,ArcmapForNetworkList));
            CCG.setSDG11(CCG.JudgeSDGTypeAndChange("11",1,ArcmapForNetworkList));
            CCG.setSDG12(CCG.JudgeSDGTypeAndChange("12",1,ArcmapForNetworkList));
            CCG.setSDG13(CCG.JudgeSDGTypeAndChange("13",1,ArcmapForNetworkList));
            CCG.setSDG14(CCG.JudgeSDGTypeAndChange("14",1,ArcmapForNetworkList));
            CCG.setSDG15(CCG.JudgeSDGTypeAndChange("15",1,ArcmapForNetworkList));
            CCG.setSDG16(CCG.JudgeSDGTypeAndChange("16",1,ArcmapForNetworkList));
            CCG.setSDG17(CCG.JudgeSDGTypeAndChange("17",1,ArcmapForNetworkList));
            CCG.LoadDifferent();
            CountryComparedGlobe.add(CCG);


            ArcmapForNetworkByCountry CCR=new ArcmapForNetworkByCountry();
            CCR.InitClass(Integer.parseInt(WorldCountryList.get(Ai).getGeoAreaCodeID()),ArcmapForNetworkList);
            CCR.setSDG1(CCR.JudgeSDGTypeAndChange("1",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG2(CCR.JudgeSDGTypeAndChange("2",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG3(CCR.JudgeSDGTypeAndChange("3",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG4(CCR.JudgeSDGTypeAndChange("4",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG5(CCR.JudgeSDGTypeAndChange("5",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG6(CCR.JudgeSDGTypeAndChange("6",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG7(CCR.JudgeSDGTypeAndChange("7",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG8(CCR.JudgeSDGTypeAndChange("8",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG9(CCR.JudgeSDGTypeAndChange("9",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG10(CCR.JudgeSDGTypeAndChange("10",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG11(CCR.JudgeSDGTypeAndChange("11",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG12(CCR.JudgeSDGTypeAndChange("12",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG13(CCR.JudgeSDGTypeAndChange("13",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG14(CCR.JudgeSDGTypeAndChange("14",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG15(CCR.JudgeSDGTypeAndChange("15",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG16(CCR.JudgeSDGTypeAndChange("16",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.setSDG17(CCR.JudgeSDGTypeAndChange("17",CCR.getGeoRegionID(),ArcmapForNetworkList));
            CCR.LoadDifferent();
            CountryComparedRegion.add(CCR);
        }
        operateExcel.writeExcel(OutPath3,"CompareCountryToGlobal",ArcmapForNetworkByCountry.class,CountryComparedGlobe);
        operateExcel.writeExcel(OutPath3,"CompareCountryToRegion",ArcmapForNetworkByCountry.class,CountryComparedRegion);


        for(int Pi=0;Pi<CountryNandRAll.size();Pi++)
        {
            int CountryID= CountryNandRAll.get(Pi).getCountryID();
            for(int Ai=0; Ai<ArcmapForNetworkList.size();Ai++)
            {
                int AreaID=ArcmapForNetworkList.get(Ai).getAreaID();
                //先判断是不是在一个区域
                for(int Ni=0;Ni<CountryNandRAll.get(Pi).getNodeList().size();Ni++)
                {
                    String Goal=CountryNandRAll.get(Pi).getNodeList().get(Ni).getGoal();
                    if(CountryID==AreaID&&ArcmapForNetworkList.get(Ai).getGoal().equals(Goal)&&ArcmapForNetworkList.get(Ai).getType().equals("Facilitation"))
                    {
                        CountryNandRAll.get(Pi).getNodeList().get(Ni).setEigDegreeF(ArcmapForNetworkList.get(Ai).getGraphWeight());
                    }
                    if(CountryID==AreaID&&ArcmapForNetworkList.get(Ai).getGoal().equals(Goal)&&ArcmapForNetworkList.get(Ai).getType().equals("Inhibition"))
                    {
                        CountryNandRAll.get(Pi).getNodeList().get(Ni).setEigDegreeI(ArcmapForNetworkList.get(Ai).getGraphWeight());
                    }
                }
            }
        }
        for(int Pi=0;Pi<CountryNandRAll.size();Pi++)
        {
            ...
            operateExcel.writeExcel(OutPath3,CountryNandRAll.get(Pi).getCountryName()+"Node",NetNode.class, CountryNandRAll.get(Pi).getNodeList());
            operateExcel.writeExcel(OutPath3,CountryNandRAll.get(Pi).getCountryName()+"Relationship",NetRelationship.class, CountryNandRAll.get(Pi).getRelationshipList());
        }
        System.out.println("组装完成");
        List<ArcmapForNetwork> ArcmapForNetworkByCountry=new ArrayList<>();
        for(int ACI=0;ACI<ArcmapForNetworkList.size();ACI++)
        {
            if(ArcmapForNetworkList.get(ACI).getScale()==3)
            {
                ArcmapForNetwork OneCountry1=new ArcmapForNetwork();
                OneCountry1.setAreaName(ArcmapForNetworkList.get(ACI).getAreaName());
                OneCountry1.setAreaAb(ArcmapForNetworkList.get(ACI).getAreaAb());
                OneCountry1.setAreaID(ArcmapForNetworkList.get(ACI).getAreaID());
                OneCountry1.setScale(ArcmapForNetworkList.get(ACI).getScale());
                OneCountry1.setGoal(ArcmapForNetworkList.get(ACI).getGoal());
                OneCountry1.setGraphWeight(ArcmapForNetworkList.get(ACI).getGraphWeight());
                OneCountry1.setType(ArcmapForNetworkList.get(ACI).getType());
                OneCountry1.setGeoRegionID(ArcmapForNetworkList.get(ACI).getGeoRegionID());
                OneCountry1.setEconomic(ArcmapForNetworkList.get(ACI).getEconomic());
                OneCountry1.setPrioritize(ArcmapForNetworkList.get(ACI).getPrioritize());

                //添加分区尺度优先目标
                for(int ACJ=0;ACJ<ArcmapForNetworkList.size();ACJ++)
                {

                    if(OneCountry1.getGeoRegionID()==ArcmapForNetworkList.get(ACJ).getAreaID()&&OneCountry1.getGoal().equals(ArcmapForNetworkList.get(ACJ).getGoal())&&
                    ArcmapForNetworkList.get(ACJ).getType().equals("Facilitation")){//这里有点问题
                        OneCountry1.setRegionalPrioritize(ArcmapForNetworkList.get(ACJ).getPrioritize());
                    }
                }
                //添加全球优先目标
                for(int ACJ=0;ACJ<ArcmapForNetworkList.size();ACJ++)
                {
                    if(ArcmapForNetworkList.get(ACJ).getScale()==0&&OneCountry1.getGoal().equals(ArcmapForNetworkList.get(ACJ).getGoal())&&
                            ArcmapForNetworkList.get(ACJ).getType().equals("Facilitation")){
                        OneCountry1.setGlobalPrioritize(ArcmapForNetworkList.get(ACJ).getPrioritize());
                    }
                }
                if(OneCountry1.getGlobalPrioritize()!=null&&OneCountry1.getPrioritize()!=null)
                {
                    if(OneCountry1.getGlobalPrioritize().equals(OneCountry1.getPrioritize())&&OneCountry1.getGlobalPrioritize().equals("FP"))
                    {
                        OneCountry1.setGlobalToNationalPrioritize("UnChange_F");
                        OneCountry1.setGN_FacilitatingConsistency(1);
                    }else if(OneCountry1.getGlobalPrioritize().equals(OneCountry1.getPrioritize())&&OneCountry1.getGlobalPrioritize().equals("IP")){
                        OneCountry1.setGlobalToNationalPrioritize("UnChange_I");
                        OneCountry1.setGN_InhibitingConsistency(1);
                        System.out.println("test");
                    }else if(OneCountry1.getGlobalPrioritize().equals(OneCountry1.getPrioritize())&&OneCountry1.getGlobalPrioritize().equals("NP")){
                        OneCountry1.setGlobalToNationalPrioritize("UnChange_N");
                    }
                    else
                    {
                        //这里面还要进一步的判断
                        if(OneCountry1.getGlobalPrioritize().equals("FP")&&OneCountry1.getPrioritize().equals("NP"))
                        {
                            OneCountry1.setGlobalToNationalPrioritize("Change_FN");
                        }
                        if(OneCountry1.getGlobalPrioritize().equals("FP")&&OneCountry1.getPrioritize().equals("IP"))
                        {
                            OneCountry1.setGlobalToNationalPrioritize("Change_FI");
                        }
                        if(OneCountry1.getGlobalPrioritize().equals("NP")&&OneCountry1.getPrioritize().equals("FP"))
                        {
                            OneCountry1.setGlobalToNationalPrioritize("Change_NF");
                        }
                        if(OneCountry1.getGlobalPrioritize().equals("NP")&&OneCountry1.getPrioritize().equals("IP"))
                        {
                            OneCountry1.setGlobalToNationalPrioritize("Change_NI");
                        }
                        if(OneCountry1.getGlobalPrioritize().equals("IP")&&OneCountry1.getPrioritize().equals("FP"))
                        {
                            OneCountry1.setGlobalToNationalPrioritize("Change_IF");
                        }
                        if(OneCountry1.getGlobalPrioritize().equals("IP")&&OneCountry1.getPrioritize().equals("NP"))
                        {
                            OneCountry1.setGlobalToNationalPrioritize("Change_IN");
                        }
                    }
                }
                if(OneCountry1.getRegionalPrioritize()!=null&&OneCountry1.getPrioritize()!=null)
                {
                    if(OneCountry1.getRegionalPrioritize().equals(OneCountry1.getPrioritize())&&OneCountry1.getRegionalPrioritize().equals("FP"))
                    {
                        OneCountry1.setRegionToNationalPrioritize("UnChange_F");
                        OneCountry1.setRN_FacilitatingConsistency(1);
                    }else if(OneCountry1.getRegionalPrioritize().equals(OneCountry1.getPrioritize())&&OneCountry1.getRegionalPrioritize().equals("IP")){
                        OneCountry1.setRegionToNationalPrioritize("UnChange_I");
                        OneCountry1.setRN_InhibitingConsistency(1);
                    }else if(OneCountry1.getRegionalPrioritize().equals(OneCountry1.getPrioritize())&&OneCountry1.getRegionalPrioritize().equals("NP")){
                        OneCountry1.setRegionToNationalPrioritize("UnChange_N");
                    }else
                    {
                        if(OneCountry1.getRegionalPrioritize().equals("FP")&&OneCountry1.getPrioritize().equals("NP"))
                        {
                            OneCountry1.setRegionToNationalPrioritize("Change_FN");
                        }
                        if(OneCountry1.getRegionalPrioritize().equals("FP")&&OneCountry1.getPrioritize().equals("IP"))
                        {
                            OneCountry1.setRegionToNationalPrioritize("Change_FI");
                        }
                        if(OneCountry1.getRegionalPrioritize().equals("NP")&&OneCountry1.getPrioritize().equals("FP"))
                        {
                            OneCountry1.setRegionToNationalPrioritize("Change_NF");
                        }
                        if(OneCountry1.getRegionalPrioritize().equals("NP")&&OneCountry1.getPrioritize().equals("IP"))
                        {
                            OneCountry1.setRegionToNationalPrioritize("Change_NI");
                        }
                        if(OneCountry1.getRegionalPrioritize().equals("IP")&&OneCountry1.getPrioritize().equals("FP"))
                        {
                            OneCountry1.setRegionToNationalPrioritize("Change_IF");
                        }
                        if(OneCountry1.getRegionalPrioritize().equals("IP")&&OneCountry1.getPrioritize().equals("NP"))
                        {
                            OneCountry1.setRegionToNationalPrioritize("Change_IN");
                        }
                    }
                }
                ArcmapForNetworkByCountry.add(OneCountry1);



                ArcmapForNetwork OneCountry2=new ArcmapForNetwork();
                OneCountry2.setAreaName(ArcmapForNetworkList.get(ACI).getAreaName());
                OneCountry2.setAreaAb(ArcmapForNetworkList.get(ACI).getAreaAb());
                OneCountry2.setAreaID(ArcmapForNetworkList.get(ACI).getAreaID());
                OneCountry2.setScale(ArcmapForNetworkList.get(ACI).getScale());
                OneCountry2.setGoal(ArcmapForNetworkList.get(ACI).getGoal());
                OneCountry2.setGraphWeight(ArcmapForNetworkList.get(ACI).getGraphWeight());
                OneCountry2.setType(ArcmapForNetworkList.get(ACI).getType());
                OneCountry2.setGeoRegionID(ArcmapForNetworkList.get(ACI).getGeoRegionID());
                OneCountry2.setEconomic(ArcmapForNetworkList.get(ACI).getEconomic());
                OneCountry2.setPopulation(ArcmapForNetworkList.get(ACI).getPopulation());
                OneCountry2.setPrioritize(ArcmapForNetworkList.get(ACI).getPrioritize());
                for(int ACJ=0;ACJ<ArcmapForNetworkList.size();ACJ++)
                {

                    if(OneCountry2.getGeoRegionID()==ArcmapForNetworkList.get(ACJ).getAreaID()&&OneCountry2.getGoal().equals(ArcmapForNetworkList.get(ACJ).getGoal())&&
                            ArcmapForNetworkList.get(ACJ).getType().equals("Inhibition")){//这里有点问题
                        OneCountry2.setRegionalPrioritize(ArcmapForNetworkList.get(ACJ).getPrioritize());
                    }
                }
                //添加全球优先目标
                for(int ACJ=0;ACJ<ArcmapForNetworkList.size();ACJ++)
                {
                    if(ArcmapForNetworkList.get(ACJ).getScale()==0&&OneCountry2.getGoal().equals(ArcmapForNetworkList.get(ACJ).getGoal())&&
                            ArcmapForNetworkList.get(ACJ).getType().equals("Inhibition")){
                        OneCountry2.setGlobalPrioritize(ArcmapForNetworkList.get(ACJ).getPrioritize());
                    }
                }
                if(OneCountry2.getGlobalPrioritize()!=null&&OneCountry2.getPrioritize()!=null)
                {
                    if(OneCountry2.getGlobalPrioritize().equals(OneCountry2.getPrioritize())&&OneCountry2.getGlobalPrioritize().equals("FP"))
                    {
                        OneCountry2.setGlobalToNationalPrioritize("UnChange_F");
                        OneCountry2.setGN_FacilitatingConsistency(1);
                    }else if(OneCountry2.getGlobalPrioritize().equals(OneCountry2.getPrioritize())&&OneCountry2.getGlobalPrioritize().equals("IP")){
                        OneCountry2.setGlobalToNationalPrioritize("UnChange_I");
                        OneCountry2.setGN_InhibitingConsistency(1);
                    }else if(OneCountry2.getGlobalPrioritize().equals(OneCountry2.getPrioritize())&&OneCountry2.getGlobalPrioritize().equals("NP")){
                        OneCountry2.setGlobalToNationalPrioritize("UnChange_N");
                    }
                    else
                    {
                        //这里面还要进一步的判断
                        if(OneCountry2.getGlobalPrioritize().equals("FP")&&OneCountry2.getPrioritize().equals("NP"))
                        {
                            OneCountry2.setGlobalToNationalPrioritize("Change_FN");
                        }
                        if(OneCountry2.getGlobalPrioritize().equals("FP")&&OneCountry2.getPrioritize().equals("IP"))
                        {
                            OneCountry2.setGlobalToNationalPrioritize("Change_FI");
                        }
                        if(OneCountry2.getGlobalPrioritize().equals("NP")&&OneCountry2.getPrioritize().equals("FP"))
                        {
                            OneCountry2.setGlobalToNationalPrioritize("Change_NF");
                        }
                        if(OneCountry2.getGlobalPrioritize().equals("NP")&&OneCountry2.getPrioritize().equals("IP"))
                        {
                            OneCountry2.setGlobalToNationalPrioritize("Change_NI");
                        }
                        if(OneCountry2.getGlobalPrioritize().equals("IP")&&OneCountry2.getPrioritize().equals("FP"))
                        {
                            OneCountry2.setGlobalToNationalPrioritize("Change_IF");
                        }
                        if(OneCountry2.getGlobalPrioritize().equals("IP")&&OneCountry2.getPrioritize().equals("NP"))
                        {
                            OneCountry2.setGlobalToNationalPrioritize("Change_IN");
                        }
                    }
                }
                if(OneCountry2.getRegionalPrioritize()!=null&&OneCountry2.getPrioritize()!=null)
                {
                    if(OneCountry2.getRegionalPrioritize().equals(OneCountry2.getPrioritize())&&OneCountry2.getRegionalPrioritize().equals("FP"))
                    {
                        OneCountry2.setRegionToNationalPrioritize("UnChange_F");
                        OneCountry2.setRN_FacilitatingConsistency(1);
                    }else if(OneCountry2.getRegionalPrioritize().equals(OneCountry2.getPrioritize())&&OneCountry2.getRegionalPrioritize().equals("IP")){
                        OneCountry2.setRegionToNationalPrioritize("UnChange_I");
                        OneCountry2.setRN_InhibitingConsistency(1);
                    }else if(OneCountry2.getRegionalPrioritize().equals(OneCountry2.getPrioritize())&&OneCountry2.getRegionalPrioritize().equals("NP")){
                        OneCountry2.setRegionToNationalPrioritize("UnChange_N");
                    }else
                    {
                        if(OneCountry2.getRegionalPrioritize().equals("FP")&&OneCountry2.getPrioritize().equals("NP"))
                        {
                            OneCountry2.setRegionToNationalPrioritize("Change_FN");
                        }
                        if(OneCountry2.getRegionalPrioritize().equals("FP")&&OneCountry2.getPrioritize().equals("IP"))
                        {
                            OneCountry2.setRegionToNationalPrioritize("Change_FI");
                        }
                        if(OneCountry2.getRegionalPrioritize().equals("NP")&&OneCountry2.getPrioritize().equals("FP"))
                        {
                            OneCountry2.setRegionToNationalPrioritize("Change_NF");
                        }
                        if(OneCountry2.getRegionalPrioritize().equals("NP")&&OneCountry2.getPrioritize().equals("IP"))
                        {
                            OneCountry2.setRegionToNationalPrioritize("Change_NI");
                        }
                        if(OneCountry2.getRegionalPrioritize().equals("IP")&&OneCountry2.getPrioritize().equals("FP"))
                        {
                            OneCountry2.setRegionToNationalPrioritize("Change_IF");
                        }
                        if(OneCountry2.getRegionalPrioritize().equals("IP")&&OneCountry2.getPrioritize().equals("NP"))
                        {
                            OneCountry2.setRegionToNationalPrioritize("Change_IN");
                        }
                    }
                }
                ArcmapForNetworkByCountry.add(OneCountry2);

            }
        }
        operateExcel.writeExcel(OutPath3,"NetPrioritizeCompare",ArcmapForNetwork.class, ArcmapForNetworkByCountry);
        ...
        //统计有数据的目标对信息
        List<HaveDataSDG> HaveDataSDGList=new ArrayList<>();

        //Data S1
        //所有的原数数据存在WorldDifferenceLevel变量中
        HSSFWorkbook DataS1 = new HSSFWorkbook();
        for(int Ai=0;Ai<WorldDifferenceLevel.size();Ai++){
            HaveDataSDG DataCountry=new HaveDataSDG();
            DataCountry.setName(WorldDifferenceLevel.get(Ai).getAggregation().getCountryName());
            DataCountry.setScale(WorldDifferenceLevel.get(Ai).getLevel());
            //记录sheet为国家名
            ...
            if(WorldDifferenceLevel.get(Ai).getAggregation().getIndivisible().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getIndivisible().size();i++){
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getIndivisible().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getIndivisible().get(i).get(6));
                }
            ...
            if(WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingOnly().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingOnly().size();i++){
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingOnly().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingOnly().get(i).get(6));
                }
            }
            //先促进后抑制
            if(WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingThenCounteracting().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingThenCounteracting().size();i++){
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingThenCounteracting().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getReinforcingThenCounteracting().get(i).get(6));
                }
            }
            //先抑制后促进
            if(WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingThenReinforcing().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingThenReinforcing().size();i++){
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingThenReinforcing().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingThenReinforcing().get(i).get(6));
                }
            }
            //单向抑制
            if(WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingOnly().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingOnly().size();i++){
                    HSSFRow row =(HSSFRow)sheet.createRow(Length);
                    if(new SDGIndicatorTool().JudgeRawData(WorldDifferenceLevel.get(Ai).getAggregation().getCountryAb(),
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingOnly().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getCounteractingOnly().get(i).get(6));
                }
            ...
            if(WorldDifferenceLevel.get(Ai).getAggregation().getCancelling().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getCancelling().size();i++){
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getCancelling().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getCancelling().get(i).get(6));
                }
            }
            //中性作用
            if(WorldDifferenceLevel.get(Ai).getAggregation().getNeuralInteraction().size()>0){
                for(int i=1;i<WorldDifferenceLevel.get(Ai).getAggregation().getNeuralInteraction().size();i++){
                    ...
                    DataCountry.getHaveDataX_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getNeuralInteraction().get(i).get(0));
                    DataCountry.getHaveDataY_Repeat().add(WorldDifferenceLevel.get(Ai).getAggregation().getNeuralInteraction().get(i).get(6));
                }
            }
            DataCountry.removeDuplicates();
            HaveDataSDGList.add(DataCountry);
        }
        ...
        // 关闭流
        out.close();
        System.out.println("代码完成");
        operateExcel.writeExcel(OutPath4,"HaveDataSDGFile",HaveDataSDG.class,HaveDataSDGList);
